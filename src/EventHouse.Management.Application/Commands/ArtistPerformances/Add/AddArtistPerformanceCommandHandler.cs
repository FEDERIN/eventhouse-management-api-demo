...
{
    public Task<ArtistPerformanceDto> Handle(
        ...
        return resilience.ExecuteSqlAsync(
            async ct =>
            {
                var eventVenueCalendar =
                    await eventVenueCalendarRepository
                        .GetByIdWithPerformancesAsync(
                            request.EventVenueCalendarId,
                            ct)
                ...
                var artistExists =
                    await artistRepository.ExistsAsync(
                        request.ArtistId,
                        ct);
                ...
                if (eventVenueCalendar.Status ==
                        EventVenueCalendarStatus.Published &&
                    request.SetStart.HasValue &&
                    request.SetEnd.HasValue)
                {
                    var isBusy =
                        await artistPerformanceRepository
                            .IsArtistBusyAsync(
                                request.ArtistId,
                                null,
                    ...
                    if (isBusy)
                        throw new ConflictException(
                            "ARTIST_UNAVAILABLE",
                            "Artist Conflict",
                            "The artist is already booked in another event.");
                }

                var domainResult =
                    eventVenueCalendar.AddPerformance(
                        request.ArtistId,
                        request.IsHeadliner,
                        request.SetStart,
                        request.SetEnd);
                ...
                if (domainResult == AddCalendarOutcome.NoChange)
                    throw new ConflictException(
                        ...
                        "Artist Conflict",
                        "The artist is already added to this event.");

                await eventVenueCalendarRepository.UpdateAsync(
                    eventVenueCalendar,
                    ct);

                var newPerformance =
                    eventVenueCalendar.Performances
                        .First(p => p.ArtistId == request.ArtistId);

                return ArtistPerformanceMapper.ToDto(
                    newPerformance);
            },
            ct);
    }
